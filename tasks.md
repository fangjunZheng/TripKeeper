...
- **司机端（/login、/home/** 等）：以移动端优先（375–430px 宽）为主，保证在手机浏览器中不需要横向滚动即可完整使用。
- **管理端（/admin/** 等）：以桌面端优先（≥1280px 宽）为主，但在 1024px 以内仍需保证基础可用性（可纵向滚动，重要信息不溢出）。
- 使用 **flex/栅格 + 自适应宽度**，避免写死像素宽度，尽量使用 `max-w-*`、`w-full`、`min-h-screen` 等工具类。
- 表单控件和按钮默认高度不低于 **44px**，方便点击。
- 列表、卡片在手机上**单列展示**，在管理端桌面视图中可根据需要扩展为多列布局。
